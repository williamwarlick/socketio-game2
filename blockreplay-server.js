...
class GameServer {
    ...
    async generateSinglePlayerRounds() {
        ...
        let rounds = [];
        let ids = Object.keys(nine_games_data_exp2);
        console.log("keys: " + ids);
        let order = await this.generateGameOrder(ids)

        for (let i = 0; i < 8; i++) {
            let id = order[i][0]
            let round = nine_games_data_exp2[id];
            round.id = id;
            round.stoppingPoint = order[i][2];
            // perfer floor or ceil? Ask about this
            round.stoppingPointNum = Math.floor( generateStoppingPoint(order[i][2], round)); 
            rounds.push(round);
        }

        function generateStoppingPoint(stoppingPoint, round) {
            let pect = [.33, .5, .8];
            if (stoppingPoint === 'Beginning') {
                return round.total_moves * pect[0];
            } else if (stoppingPoint === 'Middle') {
                return round.total_moves * pect[1];
            } else if (stoppingPoint === 'End') {
                return round.total_moves * pect[2];
            }
        }
    ...
    async generateGameOrder(ids){
        const versions = [
        [
            [ids[0], 'Move', 'Beginning'],
            [ids[1], 'Fill', 'Middle'],
            [ids[2], 'Cover', 'End'],
            [ids[3], 'Move', 'Middle'],
            [ids[4], 'Fill', 'End'],
            [ids[5], 'Cover', 'Beginning'],
            [ids[6], 'Move', 'End'],
            [ids[7], 'Fill', 'Beginning'],
            [ids[8], 'Cover', 'Middle']
        ],
        [
            [ids[3], 'Move', 'Beginning'],
            [ids[4], 'Fill', 'Middle'],
            [ids[5], 'Cover', 'End'],
            [ids[6], 'Move', 'Middle'],
            [ids[7], 'Fill', 'End'],
            [ids[8], 'Cover', 'Beginning'],
            [ids[0], 'Move', 'End'],
            [ids[1], 'Fill', 'Beginning'],
            [ids[2], 'Cover', 'Middle']
        ],
        [
            [ids[6], 'Move', 'Beginning'],
            [ids[7], 'Fill', 'Middle'],
            [ids[8], 'Cover', 'End'],
            [ids[0], 'Move', 'Middle'],
            [ids[1], 'Fill', 'End'],
            [ids[2], 'Cover', 'Beginning'],
            [ids[3], 'Move', 'End'],
            [ids[4], 'Fill', 'Beginning'],
            [ids[5], 'Cover', 'Middle']
        ]];
    ...
    async joinBlockReplay(io, socket, playerId, sonaId) {
        ...
        console.log('Player joined single player game, starting game ...' + playerId + ' ' + newGame.players[0].id + ' ' + newGame.id);
